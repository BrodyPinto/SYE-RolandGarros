...
fed_table <- federer_2021 |>
  ...
  select(player1, player2, game_score, set_score, match_score, atp_importance, atp_is_important, breakPoint) |>
  rename() |> 
  slice(334:341)
...
```{r, warning=FALSE, echo=FALSE, fig.width=15.85, fig.height=8.5, fig.cap="Blue dots → serve locations on break points; grey dots → serve locations on non break points. Dots above the dashed line → Deuce side serves; dots below the dashed line → Ad side serves"}
...
```{r, warning=FALSE, echo=FALSE, fig.width=15.85, fig.height=8.5, fig.cap="Blue dots → serve locations on important points, grey dots → serve locations on non-important points."}
...
return_stats <- djokovic_2021_returns |>
  ...
  rename(
    `Important Point` = atp_is_important,
